...
fn prints_version() -> TestResult {
    let mut cmd = Command::cargo_bin("rerobots")?;
...
#[test]
fn prints_help_search() -> TestResult {
    let mut cmd = Command::cargo_bin("rerobots")?;
    let assert = cmd.arg("help").arg("search").assert();
    let output = assert.get_output().clone();
    assert.success();
    insta::assert_snapshot!("prints_help_search", String::from_utf8(output.stdout)?);
...
#[test]
fn prints_help_launch() -> TestResult {
    let mut cmd = Command::cargo_bin("rerobots")?;
    let assert = cmd.arg("help").arg("launch").assert();
    let output = assert.get_output().clone();
    assert.success();
    insta::assert_snapshot!("prints_help_launch", String::from_utf8(output.stdout)?);
